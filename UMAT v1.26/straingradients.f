...
!     Kuksenko, V., Roberts, S. and Tarleton, E., 2019. International Journal of Plasticity, 116, pp.62-80.
     ...
     + statev_Fp, statev_gmatinv, statev_gnd, statev_gnd_t
...
!
      real(8) :: dirs(maxnslip,3)
      real(8) :: tras(maxnslip*2,3)
      real(8) :: Bmat(maxnslip*2+9,maxnslip*2+9)
      real(8) :: rhoGND(maxnslip*2)
      real(8) :: drhoGND(maxnslip*2)
!
      real(8) :: grad_invN(3,numpt), grad(3,3,3)
      ...
      integer :: i, j, k, l
      integer :: ie, ip, is
      real(8) :: y(maxnslip*2), x(maxnslip*2)
!
!
!
!
...
!
!
!             Calculate Bmatrix - using KKT solution
!             Arsenlis, A. and Parks, D.M., 1999. Acta materialia, 47(5), pp.1597-1611.
              call calculateBmatKKT(nslip,nscrew,screw,
     + dirs(1:nslip,:),tras(1:nslip,:),burgerv(1:nslip),
     + Bmat(1:nslip+nscrew+9,1:nslip+nscrew+9))
!
!
!             Use gradients per integration point
              if (gndhomogenization == 0) then
...
!
!
!             Assign Lambda to the null vector
              y = 0.
              x = 0.
              do i = 1,9
                  y(i+nslip+nscrew) = Lambda_vec(i)
              end do
!
!             Solve!
              x(1:nslip+nscrew+9) = matmul(
     + Bmat(1:nslip+nscrew+9,1:nslip+nscrew+9),
     + y(1:nslip+nscrew+9))
!
!
...
!             Cumulative slip per slip system
              totslip(1:nslip) = statev_gammasum(ie,ip,1:nslip)
!
!
              do is = 1, nslip
!                 Transform slip directions to sample reference
...
!             Calculate Bmatrix - using singular value decomposition
!             Arsenlis, A. and Parks, D.M., 1999. Acta materialia, 47(5), pp.1597-1611.
              call calculateBmatPINV(nslip,nscrew,screw,dirs(1:nslip,:),
     + tras(1:nslip,:),burgerv(1:nslip),totslip(1:nslip),
     + Bmat(1:nslip+nscrew,1:9))
!
!
!             Use gradients per integration point
...
!
!     GND model-5
!     GND calculation using the norm of || curl x (n^a*Fp*gammadot^a) ||
!     Incremental or rate form
!     followed by direct projections
!     Dai, H., 1997. Doctoral dissertation, Massachusetts Institute of Technology.
      subroutine gndmodel5(dt)
      use userinputs, only : maxnslip,
     + gndhomogenization, gndthreshold
      use globalvariables, only : numel, numdim, numpt, nnpel,
     + phaseid, numslip_all, numscrew_all, eijk, I3,
     + dirc_0_all, norc_0_all, burgerv_all, slip2screw_all,
     + gradip2ip, statev_curvature, statev_Fp,
     + statev_gammadot, statev_gmatinv, 
     + statev_gnd_t, statev_gnd,
     + statev_Lambda, statev_Lambda_t
      use utilities, only: matvec9, vecmat9
      implicit none
...
!     the NUMBER OF SLIP SYSTEMS can alter within the mesh
      ...
      real(8) :: norc_0(maxnslip,3)
      real(8) :: slip2screw(maxnslip,maxnslip)
      real(8) :: drhoGNDes(numpt,maxnslip)
      real(8) :: drhoGND(maxnslip*2)
      real(8) :: burgerv(maxnslip)
      real(8) :: vec_ip(numpt,3), n_a(3)
      real(8) :: dirs_ip(numpt,3)
!     Incompatibility
      real(8) :: dLambda_ip(numpt,3,3)
      real(8) :: dLambda(3,3), dLambda_vec(9)
      real(8) :: Lambda(3,3), Lambda_vec(9), trace
      real(8) :: kappa(3,3), kappa_vec(9)
!     Overall gradient mapping
      real(8) :: grad_invN(3,numpt), grad(3,3)
      real(8) :: lambdadot_a(3), sum
      integer :: i, j, k, l
      integer :: ie, ip, is
!
!
...
!
!
!                 gnd increment - the same for edge and screw dislocations                 
                  drhoGNDes(ip,is) = 1. / burgerv(is) *
     + norm2(lambdadot_a) * dt / (nslip+nscrew)
!
!
!
!
...
!
!             Edge dislocations
              drhoGND(1:nslip) = drhoGNDes(ip,1:nslip)              
!
!             Screw dislocations
              drhoGND(nslip+1:nslip+nscrew) = matmul(
     + slip2screw(1:nscrew,1:nslip),drhoGNDes(ip,1:nslip))
!
!             Check for a threshold
              do is =1,nslip+nscrew
                  if (abs(drhoGND(is))<gndthreshold) then
...
!     Gerken, J.M. and Dawson, P.R., 2008. Journal of the Mechanics and Physics of Solids, 56(4), pp.1651-1672.
      subroutine gndmodel6(dt)
      use userinputs, only : maxnslip,
     + gndhomogenization, gndthreshold
      use globalvariables, only : I3, numdim,
     + numel, numpt, nnpel,
     + phaseid, numslip_all, numscrew_all,
...
!
     ...
     + - dot_s * dirs_ip(ip,i) * tras_ip(ip,j)
     + + dot_t * dirs_ip(ip,i) * dirs_ip(ip,j)
!
!                         
                      end do
                  end do
...
!
!     Calculate Bmatrix using singular value decomposition
      subroutine calculateBmatPINV(nslip,nscrew,screw,
     + dirs,tras,burgerv,totslip,BmatPINV)
...
!     Burgers vector
      real(8), dimension(nslip), intent(in) :: burgerv
!     Cumulative slip per slip system
      real(8), dimension(nslip), intent(in) :: totslip
!     Rank Deficit B-matrix
...
!     For edge type of dislocations
      do is = 1, nslip
!
          if (abs(totslip(is))<sqrt(smallnum)) then
!
