...
!
!
!         Explicit time integration of states (Dunne)
          if (stateupdate==0) then
!
!
!
!             Solve using original crystal plasticity solver 
...
!
!
!         Semi-implicit time integration of states (modified Dunne)
...
!
!
!         Fully-implicit time integration of states (Huang)
...
!         If stress-based Dunne crystal plasticity DOES NOT CONVERGE
          if ((stateupdate<2).and.(cpconv == 0)) then
!             Use the fully-implicit solver relying on slip increments
...
!     Forest projections
     ...
     + abs(matmul(slip2screw,ssd))) ! screws
!     Absolute is taken since slip2screw can retain negative numbers
!
!
      rhotot = ssd +
     + abs(gnd(1:nslip)) + ! edges
     + abs(matmul(transpose(slip2screw), gnd(nslip+1:nslip+nscrew))) ! screws
!     Absolute is taken since slip2screw can retain negative numbers
!
!
!   
