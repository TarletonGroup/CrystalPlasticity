...
!
!
      subroutine sinhslip(Schmid,SchmidxSchmid,
     + abstau,signtau,tauc,rhofor,
...
!         c/a ratio correction for HCP
          if(iphase == 3) then
!             Corrected by Alvaro - 23.02.2023
...
!         This is critical threshold
          if (abstau(is) >= tauc(is)) then
!
!             slip rate
              gammadot(is) = alpha*
     + sinh(beta*(abstau(is)-tauc(is)))*signtau(is)
!
!
...
!
     ...
     + cosh(beta*(abstau(is)-tauc(is)))*signtau(is)
!
!
...
!
      subroutine doubleexpslip(Schmid,SchmidxSchmid,
     + abstau,signtau,tauc,
     + burgerv,dt,nslip,iphase,T,slipparam,
     + irradiationmodel,irradiationparam,
     + cubicslip,caratio,
     + Lp,Pmat,gammadot,dgammadot_dtau,
     + dgammadot_dtauc)
!
      use userinputs, only : useaveragestatevars, maxnparam
      use globalvariables, only : KB
...
!     variables used within this subroutine       
      integer :: is
      real(8) :: gammadot0, p, q, Foct, Fcub, DeltaF, x
!
!
!     Obtain slip parameters
...
!         Avoid negative values before elevating to power q
          if (x >= 1.0) then
!
              gammadot(is) = signtau(is)*gammadot0 !*exp(-dF/(kB*CurrentTemperature))
!
!         Standard case
          else
!
...
!             Strain rate due to thermally activated glide (rate dependent plasticity)
              gammadot(is) = signtau(is)*gammadot0*
     + exp(-(DeltaF/KB/T)*(1.-x**p)**q)
!
!
!
          endif
...
!
! 
      subroutine powerslip(Schmid,SchmidxSchmid,
     + abstau,signtau,tauc,
     + burgerv,dt,nslip,iphase,T,slipparam,
     + irradiationmodel,irradiationparam,
     + cubicslip,caratio,
...
!     Variables used within this subroutine
      integer :: is, i, j
      real(8) :: gammadot0, power_n, constant_n, slope_n, x
!
!
!     Obtain slip parameters
!     Reference strain rate
...
!
          gammadot(is) = gammadot0*x**power_n*signtau(is)
!
!
!
          dgammadot_dtau(is) = gammadot0*power_n
     + /tauc(is)*x**(power_n-1.0)
!
!
          dgammadot_dtauc(is) = -gammadot0*power_n
     + /tauc(is)*x**(power_n)*signtau(is)
!
!
          Pmat = Pmat +
     + dt*dgammadot_dtau(is)*SchmidxSchmid(is,:,:)
...
!
!     11/05/2023
!     This subroutine is prepared by Chris Hardie
!     For predicting stress using inverse-slip scheme
      subroutine sinhslipreverse(Schmid, signtau,
     + abstau,tau,tauc,rhofor,
     + burgerv,dt,nslip,iphase,T,slipparam,
     + irradiationmodel,irradiationparam,
     + cubicslip,caratio,
