...
!     4. assigns the results to the global state variables      
...
!
     ...
     + statev_totgammasum_t, statev_totgammasum, statev_iter,
     + statev_tausolute_t, statev_tausolute, statev_backstress,
     + forestproj_all, numslip_all, numscrew_all, phaseid_all, 
     ...
     + nstatv_outputs, materialid, dt_t
!
...
!     Crystal orientations at former time step
      ...
      sigmab_t = statev_backstress(noel,npt,:)
...
!     Calculate total and forest density
     ...
     + gnd_t(1:nslip+nscrew), ssd_t(1:nslip), 
     + ssdtot_t, forest_t(1:nslip), 
     + forestproj(1:nslip,1:nslip+nscrew),
     + slip2screw(1:nscrew,1:nslip), rhotot_t(1:nslip),
     + sumrhotot_t, rhofor_t(1:nslip))
...
!     Calculate crss
     ...
     + sintmat2(1:nslip,1:nslip), tauc_t(1:nslip), 
     + rhotot_t(1:nslip), sumrhotot_t, 
     + rhofor_t(1:nslip), substructure_t,
     + tausolute_t, loop_t, 
     + hmodel, hparam, imodel, iparam,
     + mattemp, tauceff_t(1:nslip))
...
!      domega(1) = W(2,1) - W(1,2)
!      domega(2) = W(1,3) - W(3,1)
!      domega(3) = W(3,2) - W(2,3)
...
!         For NO SLIP condition
          ...
          gammasum(1:nslip)=gammasum_t(1:nslip)
          gammadot=0.
          tauc(1:nslip)=tauc_t(1:nslip)
          gnd(1:nslip+nscrew)=gnd_t(1:nslip+nscrew)
          ssd(1:nslip)=ssd_t(1:nslip)
          loop(1:maxnloop) = loop_t(1:maxnloop)
          ssdtot=ssdtot_t
          forest(1:nslip)=forest_t(1:nslip)
          substructure=substructure_t
          evmp=evmp_t
          tausolute=tausolute_t
          Fp(1:3,1:3)=Fp_t(1:3,1:3)
...
!         Else Forward Gradient Predictor scheme computes sigma0
     ...
     + forestproj, slip2screw, dirc_0, norc_0, 
...
!         This part is added by Chris Hardie (11/05/2023)
!         Inverse slip scheme
          elseif (predictor == 3) then              
              

      
              call CP_StressApproximator(
     + phaid, nslip, mattemp, Cs,
     + burgerv, cubicslip, caratio,
     + sigmatr, abstautr, signtautr,
     + Schmid, Schmidvec,
     + smodel, sparam,
     + imodel, iparam,
     + tauceff_t, rhofor_t, dt,
     + sigma0)
...
!     increase in tauc due to solute force
      real(8) :: dtausolute_dg(nslip)
      
!     crss increment
      real(8) :: dtauc_dg(nslip,nslip)
!
!     total ssd density increment
      real(8) :: drhotot_dg(nslip)
!
!     forest dislocation density increment
      real(8) :: drhofor_dg(nslip,nslip)
!
!     substructure dislocation density increment
      real(8) :: drhosub_dg(nslip)
!
!     ssd density increment
      real(8) :: dssd_dg(nslip,nslip)
!
!     loop density increment
      real(8) :: dloop_dg(maxnloop,nslip)        
...
!!     or close zero
!      if (detFp <= smallnum) then ! Try integrating slip over sub time increments dt/1024 CDH 03042023
!          Fp=Fp_t
...
!     Update the states using hardening laws
     ...
     + dtausolute_dg, dtauc_dg,
     + drhotot_dg, drhofor_dg,
     + drhosub_dg, dssd_dg, dloop_dg)
...
!         Recalculate total and forest density
     ...
     + nscrew, nslip, gnd_t(1:nslip+nscrew), 
     + ssd(1:nslip), ssdtot, forest(1:nslip),
     + forestproj(1:nslip,1:nslip+nscrew),
     + slip2screw(1:nscrew,1:nslip), rhotot(1:nslip),
     + sumrhotot, rhofor(1:nslip))
...
!         Calculate crss
     ...
     + gf, G12, burgerv(1:nslip),
     + sintmat1(1:nslip,1:nslip), 
     + sintmat2(1:nslip,1:nslip),
     + tauc(1:nslip), rhotot(1:nslip),
     + sumrhotot, rhofor(1:nslip),
     + substructure, tausolute, loop,
     + hardeningmodel, hardeningparam,     
     + irradiationmodel, irradiationparam,
     + mattemp, tauceff(1:nslip))
...
!     Deformed slip direction
      real(8) :: dirs_t(nslip,3)
!     Deformed slip plane normal
      real(8) :: nors_t(nslip,3)      
!
!     deviatoric strains
      real(8) :: dev
...
!     overall projection operator
      real(8) :: PO(nslip,nslip)
...
!     Set convergence flag converged!
      cpconv = 1
      
      
!     Compute the deformed slip vectors
      call rotateslipsystems(phaid,nslip,caratio,
     + gmatinv_t,dirc_0,norc_0,dirs_t,nors_t)
...
!     Calculate crss
     ...
     + mattemp, tauceff(1:nslip))
      ...
      Hab = 0.
!     Calculate derivative terms numerically
      if (hardeningmodel==1) then
          
          Hab = dtauc_dg
          
          
!     IGNORED "useaveragestates=1" - will be taken care of later!!!
      elseif (hardeningmodel==2) then
          
!         Overall projection operator
          PO = forestproj(1:nslip,1:nslip) + ! edges
     + matmul(forestproj(1:nslip,nslip+1:nslip+nscrew),slip2screw) ! screws 
          
          PO = matmul(PO,dssd_dg)
          
          do is=1,nslip
              do js=1,nslip
                  
                  Hab(is,js)=gf*G12*burgerv(is)
     + /2./dsqrt(rhofor(is))*PO(is,js)
...
!         Forest state variable
          do is=1,nslip

                  
              Hab(is,is)=gf*G12*burgerv(is)
     + /2./dsqrt(rhofor(is))*drhofor_dg(is,is)
              
              do js=1,nslip
                  
                  Hab(is,js) = Hab(is,js) +
     + hardeningparam(8)*G12*burgerv(is)/2./
     + dsqrt(substructure)*(dlog(1./burgerv(is)/dsqrt(substructure))-1.)
     + *drhosub_dg(js)
...
!!     This numerical derivative gives inaccurate results
!!     Does not work in case of cross-hardening
...
!  
!
!     This subroutine is written by Chris Hardie (11/0/2023)      
!     Explicit state update rule
!     Solution using state variables at the former time step
      subroutine CP_StressApproximator(phaid, 
     + nslip, mattemp, Cs, 
     + burgerv, cubicslip, caratio,
     + sigmatr, abstautr, signtautr, 
     + Schmid, Schmidvec,   
...
!
      use globalvariables, only : I3, I6, smallnum
!
      use userinputs, only : maxniter, maxnparam
!
      use utilities, only : matvec6
!      
      use slip, only : sinhslipreverse
!
!
      implicit none
!
...
!
!     other variables
      real(8) :: dummy6(6), active, damping, tol 
      integer :: is, iter
...
!     Build Shear stiffness matrix
      ...
      tol = 10.0
!     Newton-Raphson (NR) iteration to find stress increment
      do while (psinorm >= tol)!.and.(iter <= maxniter))
...
!         sinh law
          elseif (slipmodel == 1) then
              ...
              call sinhslipreverse(Schmid,signtau,
     + abstau,tau,tauceff_t,rhofor_t,burgerv,dt,
     + nslip,phaid,mattemp,slipparam,
     + irradiationmodel,irradiationparam,
...
C             call doubleexpslip(Schmid,SchmidxSchmid,
C     + abstau,signtau,tauceff_t,burgerv,dt,nslip,phaid,
C     + mattemp,slipparam,irradiationmodel,
C     + irradiationparam,cubicslip,caratio,
C     + Lp_s,Pmat_s, gammadot_s,
C     + dgammadot_dtau_s,dgammadot_dtauc_s) 
...
C              call powerslip(Schmid,SchmidxSchmid,
C     + abstau,signtau,tauceff_t,burgerv,dt,
C     + nslip,phaid,mattemp,slipparam,
C     + irradiationmodel,irradiationparam,
C     + cubicslip,caratio,Lp_s,Pmat_s,
C     + gammadot_s,dgammadot_dtau_s,
C     + dgammadot_dtauc_s )
...
!         rss and its sign
          ...
          do is = 1, nslip
              ...
              if (abstau(is) .GT. tauceff_t(is)) then
                  active=active+1
              end if
          end do             
...
!
!
          damping = min(2./active,0.5)
          !damping = 0.5
          !if (psinorm > 200.0) then
          !    damping=min(2.0/active, 0.5)
          !end if
...
!         plastic strain rate
          plasstrainrate = (Lp_s + transpose(Lp_s))/2.
...
!     End of NR iteration for stress approximation
      ...
      return
          end subroutine CP_StressApproximator
!      
!      
!
!
