...
!      
      use globalvariables, only: statev_outputs, nstatv_outputs
...
!     Reset number of statevariables to zero
      statev_outputs=0
...
!     17th state-variable output / number of outputs: 1
!     number of solver iterations: statev_iter
      statev_outputs(17) = 0
      
!     18th state variable output / number of outputs: 6
...
!      write(*,*) 'Please check:'
!      write(*,*) 'Number of state variables in DEPVAR option
!     + shall be equal to  ', nstatv_outputs, 'and it is set to ', nstatv
...
!     *******************************************************      
      ...
      use globalvariables, only: nstatv_outputs
...
!
      ...
      use userinputs, only : maxnslip, maxnloop
      use globalvariables, only: statev_outputs,
     + nstatv_outputs
!
	implicit none
!
...
!
     ...
     + statev_curvature, statev_backstress,
     + statev_ssdtot, statev_substructure,
     + statev_tausolute, statev_totgammasum,
     + statev_gammasum, statev_gammadot, 
     + statev_tauc, statev_ssd, 
     + statev_loop, statev_gnd,
     + statev_forest, statev_outputs, 
     + statev_iter, nstatv_outputs
      use userinputs, only: maxnslip, maxnloop
      use utilities, only: matvec6, matvec9
      implicit none
!     Element number
...
!     Elastic strain in crystal frame (x6)
      if (statev_outputs(4)==1) then
          ...
          call matvec6(statev_Eec(noel,npt,:),
     + d6)
...
!     Custom state variables 17-20
!     Please add custom outputs here!
      
      
!     State variable-17
!     Number of iterations (x 1)       
      if (statev_outputs(17)==1) then
          
          statev(i) = statev_iter(noel,npt)
