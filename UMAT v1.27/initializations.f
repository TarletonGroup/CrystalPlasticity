...
!     Subroutines that need to run once at the beginning of calculations
      ...
      use userinputs, only: readinpfile,
     + defaultnumel, defaulteltyp
      use meshprop, only : feprop
      use useroutputs, only: defineoutputs
      use fileIO, only: fileread
      use globalvariables, only: numel, eltyp
      implicit none
!
!
!     0. Get the element number and type
!     Read from *.INP file
      if (readinpfile==1) then
          call fileread
          write(*,*) '0. ABAQUS "*.INP" file has been read!'
!     Use the user inputs
      else
          eltyp = defaulteltyp
          numel = defaultnumel
          write(*,*) '0. Default element type and number is used!'
          write(*,*) 'Element type: ' // trim(defaulteltyp)
          write(*,*) 'Element number: ', defaultnumel
      end if
!
!
...
!
!
!         Reading interaction coefficients are not working for now!
!         The slots reserved are as follows:
...
!
!         PROPS(235-250) are intentionally left empty!
!
!
      endif
!
...
!     Arrays allocated
     ...
     + statev_loop, statev_loop_t, statev_backstress_t,
...
!
!
!     For multi material case with multiple grains
      allocate(featureid(numel,numpt))
      featureid=0
!
!     For multi material case with varying phases
...
!
      ...
      statev_sigma_t2=0.
      allocate(statev_backstress_t(numel,numpt,6))
