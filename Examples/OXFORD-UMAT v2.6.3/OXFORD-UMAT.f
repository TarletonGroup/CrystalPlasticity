...
!     Subroutine for initialization 
     ...
     + statev_backstress, statev_backstress_t, ip_count,
     + calculategradient
...
!
!
!                 Possible if the element has more than one integration point
                  if (calculategradient==1) then
!
...
!         element gradients are calculatable (calculategradient==1)
          if ((init_once==1).and.(calculategradient==1)) then
!
