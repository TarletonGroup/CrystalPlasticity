...
!     1. Allocate arrays
      ...
      write(*,*) '1. Array allocation completed!'
!
!
!     2. Enter mesh/element properties
!     to get number of integration points for array allocation
      call feprop
      write(*,*) '2. Mesh initialization completed!'
!
...
!         Open the file
          open(100,file=foldername // '/' // voxfilename,action='read',
     + status='old')
!
!
...
!     Subroutines that need to run once at the beginning of calculations
     ...
     +  maxnslip, maxnparam, maxnmaterial, backstressmodel,
     + readmaterialfile 
      use globalvariables, only: materialid,
     ...
     + Euler, dirc_0_all, norc_0_all,
     + trac_0_all, linc_0_all, Schmid_0_all,
     + forestproj_all, screw2slip_all,
     + slip2screw_all, screw_all,
...
!     Screw to slip system mapping
      real(8) :: screw2slip(maxnslip,maxnslip)
...
!     Reset arrays
      ...
      Schmid_0=0.
      forestproj=0.; screw=0
      slip2screw=0.; screw2slip=0.
...
!         Enter materials subroutine once for every ip to get number of slip systems    
     ...
     + gf,G12,v12,alphamat,burgerv,tauc_0,rho_0,
...
!
!         PROPS(264-300) are intentionally left empty!
!
!
      endif
!
...
!     Initialize total ssd density
      statev_ssdtot(noel,npt)=sum(rho_0(1:nslip))
      statev_ssdtot_t(noel,npt)=sum(rho_0(1:nslip))
!
!
...
!     This is needed once per element since the material is different
     ...
     + screw,dirc,norc,trac,linc,forestproj,slip2screw,screw2slip)
!
!
...
!     Arrays allocated
      ...
      use globalvariables, only : numel, numpt, numdim, materialid,
     + featureid, phaseid, phaseid_all, numslip_all, numscrew_all,
     ...
     + statev_loop, statev_loop_t, Euler,
     ...
     + sintmat1_all, sintmat2_all, 
     + hintmat1_all, hintmat2_all,
     + backstressparam_all, screw2slip_all,
...
!     Allocate slip systems
      ...
      trac_0_all=0.
      allocate(linc_0_all(maxnmaterial,maxnslip*2,3))
      linc_0_all=0.
...
!
!     Mapping for dislocations at slip sytems to screw systems for GND
      allocate(screw2slip_all(maxnmaterial,maxnslip,maxnslip))
      screw2slip_all=0.
!
...
!     Outputs
      ...
      real(8), dimension(nslip,nscrew), intent(out) :: screw2slip
