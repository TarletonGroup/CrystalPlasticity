...
!
     ...
     + statev_backstress_t, statev_backstress, screw2slip_all,
...
!     Screw to slip system map
      real(8) :: screw2slip(numslip_all(matid),numscrew_all(matid))
!
!     Schmid Dyadic
...
!     Screw to slip system mapping
      screw2slip = screw2slip_all(matid,1:nslip,1:nscrew)
!
...
!     Calculate total and forest density
     ...
     + forestproj, slip2screw, screw2slip,
...
!     Forest projections
      real(8), intent(in) :: screw2slip(nslip,nscrew)
!     Initial Schmid tensor
      real(8), intent(in) :: Schmid_0(nslip,3,3)  
!     Schmid tensor
      real(8), intent(in) :: Schmid(nslip,3,3)  
...
!
     ...
     + matmul(screw2slip, abs(gnd(nslip+1:nslip+nscrew))) ! screws
!
!
!   
!     Scalar sum
