...
!
!
!     F. Dunne stress-based solver explicit/semi-implicit state update
...
!
     ...
     + backstressmodel, inversebackup, guesstolerance,
     + stateupdate 
!
...
!
      use errors, only : error
!
      use innerloop, only : CP_innerloop, CP_innerloop_inverse
!
      implicit none
!
!
...
!     Trial shear stress variables
      real(8) :: tautr(nslip), signtautr(nslip), abstautr(nslip)
!
...
!     other variables
     ...
     + dummy33_(3,3), dummy6(6), dummy0
      integer :: is, il, iter, oiter, inv_iter
!
!
...
!
          ...
          call CP_innerloop(Schmid_0,Schmid,SchmidxSchmid,Schmidvec,
     ...
     + Dp,dstranp,sigma,sigmatr,gammadot,
     + gammasum,Lp)
      
          if (iter .GE. maxniter) then
!         did not converge
          cpconv = 0
...
!         convert stress to 3x3 marix
          call vecmat6(sigma,sigma33)       
         
          if ((cpconv == 0) .AND. (inversebackup .EQ. 1))   then ! try inverse scheme .AND. Predictor .EQ. 3
...
!             Stress at the former time step
     ...
     + irradiationmodel, irradiationparam, 
     + tauceff, rhofor, dt,
     + sigma, inv_iter, guesstolerance, X)
