...
!
!
      subroutine CP_innerloop(Schmid_0,Schmid,SchmidxSchmid,Schmidvec,
     + tau,X,abstau,signtau,tauceff,rhofor,burgerv,dt,
     + nslip,phaid,mattemp,slipparam,slipmodel,creepmodel,
     + irradiationmodel,irradiationparam,
     + cubicslip,caratio,creepparam,Cs, iter, cpconv, invdpsi_dsigma,
     + Dp,plasstraininc,sigma,sigmatr,gammadot,
     + gammasum,Lp)
      
      use globalvariables, only : I6
!
      use userinputs, only : maxniter, tolerance, 
     + maxnparam, maxnloop, quadprec, SVDinversion, maxnslip
!
      use utilities, only : matvec6,
     + nolapinverse16, nolapinverse,
     + SVDinverse 
!      
...
!     plastic strain increment
      real(8), intent(out) :: plasstraininc(6)
...
!     Solution using state variables at the former time step
      subroutine CP_innerloop_inverse(
...
!      
      use slip, only : sinhslipreverse, powerslipreverse, 
...
!      allocate(dpsi_dsigma16(6,6),l(6,6),u(6,6),p(6,6))
      debug=0
      do while (debug==1)
          debugwait = 1
      end do   
...
!     Newton-Raphson (NR) iteration to find stress increment
      do while ((psinorm >= tol))!  .OR. (psinorm2 >= tol2))!((psinorm >= tol)) !((psinorm >= tol)  .AND. (psinorm2 >= tol2))
...
!         sinh law
          elseif (slipmodel == 1) then
              ...
              call sinhslipreverse(Schmid,SchmidxSchmid,signtau,
...
!         double exponent law (exponential law)
          elseif (slipmodel == 2) then
           ...
           call doubleexpslipreverse(Schmid,SchmidxSchmid,
     + tau,X,abstau,signtau,tauceff,
     + burgerv,dt,nslip,phaid,mattemp,slipparam,
     + irradiationmodel,irradiationparam,
     + cubicslip,caratio,
     + Lp_s,Pmat,absgammadot_s,gammadot_s,dtau_dgammadot_s,dgammadot_dtau) 
...
!         power law
          ...
          call powerslipreverse(Schmid,SchmidxSchmid,
     + tau,X,abstau,signtau,tauceff,
     + burgerv,dt,nslip,phaid,mattemp,slipparam,
     + irradiationmodel,irradiationparam,
     + cubicslip,caratio,
...
!         Forward Jacobian
         ...
         ! CALL LU_DECOMP(dpsi_dsigma16, p, l, u)
...
!     End of NR iteration for stress approximation
      ...
      return
      end subroutine CP_innerloop_inverse
