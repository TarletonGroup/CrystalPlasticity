...
!
     ...
     + numslip_all, numscrew_all, phaseid_all, materialid,
     + dirc_0_all, norc_0_all, caratio_all, cubicslip_all,
     + Cc_all, gf_all, G12_all, alphamat_all, burgerv_all,
     + sintmat1_all, sintmat2_all, hintmat1_all, hintmat2_all, X_all,
...
!
     ...
     + FGpredictor, maxnslip, maxnparam, maxxcr, cutback, explicit,
     + phi, maxnloop
!
!
...
!     Fraction of strength due to hard obstacles
      real(8) :: X
...
!         get material constants
     ...
     + gf,G12,alphamat,notused1, ! state variables are NOT updated here!
     ...
     + notused7,notused8) ! Interaction matrices are not updated here
...
!     Calculate crss
     ...
     + sintmat2(1:nslip,1:nslip), X,
     + tauc_t(1:nslip), ssdtot_t, 
     + rhofor_t(1:nslip), substructure_t,
     + tausolute_t, loop_t, imodel, iparam,
     + mattemp,
     + tauceff_t(1:nslip))
...
!         Else Forward Gradient Predictor scheme computes sigma0
     ...
     + sintmat1, sintmat2, 
     + hintmat1, hintmat2, X,
     + tauceff_t, tauc_t, rhotot_t, ssdtot_t,
     ...
     + totgammasum0, evmp0,
     + tauc0, tausolute0, 
     + ssdtot0, ssd0, loop0, forest0, substructure0,
...
!
!     Fraction of strength interactions
      real(8), intent(in) :: X
!      
!     overall crss
...
!         Calculate crss
     ...
     + sintmat2(1:nslip,1:nslip), X,
     + tauc(1:nslip), ssdtot, rhofor(1:nslip),
...
!     Numerical calculation of derivative dtauc/dgamma
      ...
      do is = 1, nslip
          ...
          if (abs(gammadot(a))>1.d-8) then
          
              Hab(a,a) = (tauceff(a)-tauceff_t(a))/dt/
     + abs(gammadot(a))
...
!     sqrt(gnde*gnde +gnds*gnds) + ssd
      rhotot = sqrt(sum(gnd*gnd)) + ssdtot
