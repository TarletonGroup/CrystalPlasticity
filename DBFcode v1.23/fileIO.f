...
!     find the number of lines in a file
      ...
      implicit none
      !character(len=:), allocatable :: wd1
      !character(len=:), allocatable :: wd2
!
      integer ierr
...
!
!
      if (ierr==1) call error(9)
      
!
!     Read element number and element type from .INP file
      call read_abqinpfile(filename)
!     
!     
!
!
...
!     find the working directory
      ...
      implicit none
      character(len=:), allocatable, intent(out) :: wd1
      character(len=:), allocatable, intent(out) :: wd2
!
...
!     2. Total number of elements in the mesh
      subroutine read_abqinpfile(filename)
      use globalvariables, only: eltyp, numel
      use userinputs, only: inpfilename
      use errors, only: error
      implicit none
      character(len=:), allocatable, intent(in) :: filename
      !character(len=:), allocatable, intent(in) :: wd1
      !character(len=:), allocatable, intent(in) :: wd2
!     Variables
      character(len=256) :: line1, eltyp1
      character(len=:), allocatable :: line2
      character(len=:), allocatable :: fname
!     logical there
      integer  n, i, sline, eline, sum, pos
...
!     If not working - try definition
      if  (my_iostat/=0) then
          close(11)
          write(*,*) 'Trying the working directory 
