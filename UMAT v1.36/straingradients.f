...
!     Proposed by Chris Hardie
     ...
     + eijk, I3, statev_curvature, statev_Lambda, statev_gammadot,
...
!             Slip rate per slip system
              gammadot(1:nslip) = statev_gammadot(ie,ip,1:nslip)
!
!
              do is = 1, nslip
...
!             Arsenlis, A. and Parks, D.M., 1999. Acta materialia, 47(5), pp.1597-1611.
     ...
     + tras(1:nslip,:),burgerv(1:nslip),gammadot(1:nslip),
...
!     Calculate Bmatrix using singular value decomposition
     ...
     + dirs,tras,burgerv,gammadot,BmatPINV)
      use userinputs, only: slipratethreshold
...
!     For edge type of dislocations
      do is = 1, nslip
!
          if (abs(gammadot(is))<slipratethreshold) then
!
