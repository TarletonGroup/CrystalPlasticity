...
!     ********************************************
     ...
     + loop, rhofor,rhotot,rhosub,
     + tausolute,dtauc,drhotot,drhofor,
     + drhosub, dssd, dloop,
     + ddtauc_ddgamma)
...
!     derivative of tauc with respect to slip rate
      real(8),intent(out) :: ddtauc_ddgamma(nslip,nslip)
!
!
!
!
...
!
              do js=1,nslip
                  ddtauc_ddgamma(js,is) = Hab(js,is)*
     + h0 * (1.-tauc(is)/ss)**m*sign(1.0,gammadot(is))
              end do
!
!
          end do
...
!             Burgers vector
              ...
              do js = 1,nslip
                  dssd(is) = dssd(is) + 
     + k1/b*sqrt(Hab(is,js)*rhotot(js))
              end do
              dssd(is) = dssd(is)-k2*ssd(is)
              dssd(is) = dssd(is)*absgammadot(is)*dt
!
!
          end do
!
